...
**Shiny app url:** <https://gtex.shinyapps.io/TNBC-ST-dashboard/>
...
**Widget 1 — Cell-Type Composition Across All Slides** (stacked bar chart) - Slides ordered by descending tumor content - Hover tooltip: slide ID, tissue type, percentage of spots (%), spot count - Legend toggles to show/hide individual tissue types

**Widget 2 — Single Sample Explorer** (two sub-tabs) - *Spatial Tissue Map*: each dot is one spot colored by tissue type; hover shows spot ID, label, tumor fraction, stroma fraction, and dominance score; sidebar checkboxes filter tissue types - *Tumor Fraction Heatmap*: continuous color scale (0–1) encoding raw tumor fraction per spot; hover shows exact fractions and assigned hard label
